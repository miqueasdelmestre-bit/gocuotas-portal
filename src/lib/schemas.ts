...
export const physicalMaterialRequestSchema = z
  .object({
    ...
    cuit: z
      ...
      .refine(isValidCuit, "El CUIT ingresado no es válido"),
    confirmCuit: z.string().min(1, "Confirmá el CUIT de tu razón social"),
    email: z
      .string()
      .min(1, "Ingresá tu correo electrónico")
      .email("Ingresá un correo electrónico válido"),
  ...
  .superRefine((values, ctx) => {
    if (values.cuit !== values.confirmCuit) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "El CUIT no coincide con la confirmación",
        path: ["confirmCuit"],
      });
    }
  });
